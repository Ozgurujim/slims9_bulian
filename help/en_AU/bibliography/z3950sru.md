

#### Z39.50 SRU

<hr>
SRU Copy cataloguing allows us to retrieve the cataloguing data from an SRU server in other libraries. It is similar to Z39.50, but with no need to have YAZ library installed on the SLiMS server.
By default SLiMS points to the Library of Congress (http://loc.gov), but other sources can be added at will. 

In SLiMS 9 the configuration of cataloguing servers is done in *Master Files-> Cataloguing Servers*.

[ In version 8 this was done within the SLiMS *sysconfig.inc.php* configuration file, or more preferably within *sysconfig. local.inc.php*, so consult your server administrator if you use SLiMS 8].

Librarians simply enter the title, author or ISBN/ISSN of a resource they wish data for, then SLiMS will retrieve the data if it is held in the other library's catalogue. If there is more than one result, then the librarian is able to select the desired title and save it.

For effective searching of Z39.50 it is best to use ISBN/ISSN, because ISBN/ISSN searches will be more accurate.

Here is a list of  <a href="http://irspy.indexdata.com/" 
   onclick="const w=800;const h=600;window.open(this.href,'newwindow','width='+w+',height='+h+',left='+(screen.availWidth-w)+',top='+((screen.availHeight-h)/2)+',resizable=yes'); return false;" style="color: purple; text-decoration: underline;">
   <u>🔗z39.50 servers</u></a>

##### Usage: 

Type the title, author or ISBN. Choose the appropriate result, and save it to the database.

<hr>

<a href="https://slims.web.id/docs/user-guide/Modules/Bibliography/Copy%20Cataloging/Z3950%20SRU" 
   onclick="const w=800;const h=600;window.open(this.href,'newwindow','width='+w+',height='+h+',left='+(screen.availWidth-w)+',top='+((screen.availHeight-h)/2)+',resizable=yes'); return false;" style="color: purple; text-decoration: underline;">
   <u>🔗See Also: SLiMS online UserGuide - Z3950SRU
</u></a>

<a href="https://slims.web.id/docs/user-guide/Modules/Bibliography/Copy%20Cataloging/Configuration" 
   onclick="const w=800;const h=600;window.open(this.href,'newwindow','width='+w+',height='+h+',left='+(screen.availWidth-w)+',top='+((screen.availHeight-h)/2)+',resizable=yes'); return false;" style="color: purple; text-decoration: underline;">
   <u>🔗See Also: SLiMS online UserGuide- Copy cataloguing server configuration
</u></a>

