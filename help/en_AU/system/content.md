...
Example of usage: consider a path created for the new content that is stored as "*hints*". On a SLiMS system accessed by the URL http://localhost/slims , to display this content then we need to write the url:

http://localhost/slims/index.php?p=hints
...
* *Content title*   [Text heading, required] - identifies this content in a user friendly way
* *Publish at* [ Date, optional ] - the date the content will become public. If blank, published immediately.
* *This is news* [Yes/No , required] - if Yes, the content appears in the News menu list.
* *Path (must be unique string without spaces)* [ e.g "hints", required] - system reference ID
* *Content* - the actual content which is edited as html , using the inbuilt WYSIWYG editor which provides a wide range of functions including image insert, text formatting, and source editing ]
