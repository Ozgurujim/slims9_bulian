#### Member Type

------

This defines the types of membership. The type of membership controls:
- *Loan Limit* (limit number of borrowed copies), 
- *Loan Period* (the length of a loan), 
- *Reserve* (whether reservations can be made), 
- *Reserve Limit* (limit number of copies reserved), 
- *Membership Period* (length of membership), 
- *Reborrow Limit* (limits extensions of loans), 
- *Fine each day* (fine per day for overdues ), and 
- *Overdue Grace Period* (tolerance in days for late returns).

To add new member type, click on the **Add New Member Type** button and proceed by filling all the fields provided as mention above. 



<a href="https://slims.web.id/docs/user-guide/Modules/Membership/member%20type" 
   onclick="const w=800;const h=600;window.open(this.href,'newwindow','width='+w+',height='+h+',left='+(screen.availWidth-w)+',top='+((screen.availHeight-h)/2)+',resizable=yes'); return false;" style="color: purple; text-decoration: underline;">
   <u>🔗See Also: SLiMS online UserGuide
</u></a>

